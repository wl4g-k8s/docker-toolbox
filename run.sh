...
function build_images() {
  ...
  if [[ $build_modules == *"agentmodifier"* || $build_modules == *"all"* ]]; then
    ...
    cd $BASE_DIR/agentmodifier/book-playground/ && git reset --hard && git pull
    cd $BASE_DIR/agentmodifier/book-playground/java-playground/ && ./gradlew :playground-agent-modifier:clean build -x test
